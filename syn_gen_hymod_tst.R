...
pred_in<-predictor_mat_hist[idx,1:10]
...
saveRDS(syn_out,'out/hymod_tst_gl-sep-mv-lin-ar_syn-error_default.rds')

sim<-matrix(rep(predictor_mat_hist[idx,1],n),ncol=n,byrow=F)
swm_out<-sim+syn_out
swm_out[swm_out<0]<-0

saveRDS(swm_out,'out/hymod_tst_gl-sep-mv-lin-ar_syn-flow_default.rds')
