...
dyn_res_preds<-pred_mat_scale[idx_cal,]
dyn_res_preds_zero<-pred_mat_zero_min[idx_cal,]

#Constrained optimization for MLE
dyn_res_mle<-optimx(par=start,fn=GL_fun_mv_ar1_lin,sig_var=dyn_res_preds_zero,beta_var=dyn_res_preds,xi_var=dyn_res_preds,phi_var=dyn_res_preds_zero,et=err_db_cal,noise=noise,neg=F,
...
ll<-GL_fun_mv_ar1_lin(dyn_res_coef,sig_var=dyn_res_preds_zero,beta_var=dyn_res_preds,xi_var=dyn_res_preds,phi_var=dyn_res_preds_zero,et=err_db_cal,noise=noise,neg=F)
param_out<-GL_fun_mv_ar1_lin_params(dyn_res_coef,sig_var=dyn_res_preds_zero,beta_var=dyn_res_preds,xi_var=dyn_res_preds,phi_var=dyn_res_preds_zero,et=err_db_cal)

print(ll)
print(coef_mat)

saveRDS(dyn_res_coef,paste('fit_rev1/hymod_dyn-res-coef_',hym_site,'_v-',vers,'_seed',seed,'_cal.rds',sep=''))
saveRDS(coef_mat,paste('fit_rev1/hymod_dyn-res-coef-mat_',hym_site,'_v-',vers,'_seed',seed,'_cal.rds',sep=''))
saveRDS(param_out,paste('fit_rev1/hymod_dyn-res-params_',hym_site,'_v-',vers,'_seed',seed,'_cal.rds',sep=''))
...
noise_err_db_cal<-rnorm(length(err_db_cal),sd=0.1)
...
saveRDS(noise_pred_mat_zero,paste('fit_rev1/hymod_noise-pred-mat-zero_val_',hym_site,'_v-',vers,'_seed',seed,'_cal.rds',sep=''))

dyn_res_preds<-noise_pred_mat[idx_cal,]
dyn_res_preds_zero<-noise_pred_mat_zero_min[idx_cal,]

noise_dyn_res_mle<-optimx(par=start,fn=GL_fun_mv_ar1_lin,sig_var=dyn_res_preds_zero,beta_var=dyn_res_preds,
                          xi_var=dyn_res_preds,phi_var=dyn_res_preds_zero,et=err_db_cal,noise=noise_err_db_cal,neg=F,
                          lower = lb,upper = ub,method = 'Rvmmin',
                          control = list(maximize=T,all.methods=F))
...
noise_ll<-GL_fun_mv_ar1_lin(noise_dyn_res_coef,sig_var=dyn_res_preds_zero,beta_var=dyn_res_preds,xi_var=dyn_res_preds,phi_var=dyn_res_preds_zero,et=err_db_cal,noise=noise_err_db_cal,neg=F)
noise_param_out<-GL_fun_mv_ar1_lin_params(noise_dyn_res_coef,sig_var=dyn_res_preds_zero,beta_var=dyn_res_preds,xi_var=dyn_res_preds,phi_var=dyn_res_preds_zero,et=noise_err_db_cal)

print(noise_ll)
print(noise_coef_mat)

saveRDS(noise_dyn_res_coef,paste('fit_rev1/hymod_noise-dyn-res-coef_',hym_site,'_v-',vers,'_seed',seed,'_cal.rds',sep=''))
saveRDS(noise_coef_mat,paste('fit_rev1/hymod_noise-dyn-res-coef-mat_',hym_site,'_v-',vers,'_seed',seed,'_cal.rds',sep=''))
saveRDS(noise_param_out,paste('fit_rev1/hymod_noise-dyn-res-params_',hym_site,'_v-',vers,'_seed',seed,'_cal.rds',sep=''))
