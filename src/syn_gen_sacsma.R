...
if(noise_reg==F){
  ll_vec<-readRDS(paste('fit_rev1/sacsma_llvec_',sma_site,'_v-',vers,'.rds',sep=''))
  seed<-which.max(ll_vec)}
if(noise_reg==T){
  noise_ll_vec<-readRDS(paste('fit_rev1/sacsma_noise-llvec_',sma_site,'_v-',vers,'.rds',sep=''))
...
#-------------------------------------------------------------------------------
pred_mat<-sma_predmat_hist[,rf_idx]
...
e_t<-sma_predmat_hist[gen_idx,'err 0']
...
sim<-matrix(rep(sma_predmat_hist[gen_idx,'sim 0'],n),ncol=n,byrow=F)
swm_out<-sim+syn_out
swm_out[swm_out<0]<-0
