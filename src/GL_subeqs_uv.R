...
#Eq 4 rearranged for a_t out to AR(3)
a_tx<-function(e_t, phi_1, phi_2, phi_3, sigma_t){
  a_t<-(e_t - (c(0,e_t[-c(length(e_t))]))*phi_1 - (c(0,0,e_t[-c((length(e_t)-1),length(e_t))]))*phi_2 - (c(0,0,0,e_t[-c((length(e_t)-2),(length(e_t)-1),length(e_t))]))*phi_3)/sigma_t
  return(a_t)
}

a_txx<-function(e_t, sigma_t){
  a_t<-e_t/sigma_t
  return(a_t)
}
