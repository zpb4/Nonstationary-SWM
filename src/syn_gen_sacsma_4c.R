...
hym_predmat_4c<-readRDS(paste('data_rev1/sma_predmat_4c_',sma_site,'.rds',sep=''))
norm_vec<-readRDS(paste('fit_rev1/sacsma_norm-vec_hist_',sma_site,'.rds',sep=''))
rf_err_corr<-readRDS(paste('fit_rev1/sacsma_rf-err-corr_cal_',sma_site,'_seed',seed,'.rds',sep=''))
...
res_idx<-sort(which(colnames(sma_predmat_4c)%in%c('precip 0','tavg 0','sim 0','runoff 0','baseflow 0','et 0','swe 0','upr_sm 0','lwr_sm 0','store 0')))
...
pred_mat<-sma_predmat_4c[,rf_idx]
dyn_res_preds<-sma_predmat_4c[,res_idx]
...
e_t<-sma_predmat_4c[gen_idx,'err 0']
...
saveRDS(syn_out,paste('out_rev1/sacsma_syn-err_',sma_site,'_',gen_period,'_',samp_type,'_nreg=',noise_reg,'_',n,'X_v-',vers,'.rds',sep=''))

sim<-matrix(rep(sma_predmat_4c[gen_idx,'sim 0'],n),ncol=n,byrow=F)
swm_out<-sim+syn_out
swm_out[swm_out<0]<-0
saveRDS(swm_out,paste('out_rev1/sacsma_syn-flow_',sma_site,'_',gen_period,'_',samp_type,'_nreg=',noise_reg,'_',n,'X_v-',vers,'.rds',sep=''))
