...
png(paste('h:/oroville_non-stationary/paper/figs_rev1/si/all-mth-resids_tst_fig8_',wy_tag,'_v-',vers,'_nreg=',noise_reg,'.png',sep=''),width=768,height=896)
par(mfrow=c(4,3),oma=c(2,5,0,0),mar=c(2,4,3,0.5),mgp=c(2.5,0.5,0),tcl=-0.2,cex.lab=2,cex.axis=2,cex.main=3,font.main=4)

for(i in 1:12){
  seas<-which(ixx_comp$mon==(i-1))
  kd_syn_err<-density(syn_res[seas],n=512)
  if(i==1|i==4|i==7|i==10){dens_lab='Density'}else{dens_lab=''}
  hist(err_db[seas]*hym_kcfs_conv,breaks=brks,xlim=xlm,ylim=c(0,yupr),freq=F,main=mth[i],col='skyblue',xlab='',ylab=dens_lab,axes=F)
  axis(1,at=x_axis,labels=x_axis)
  if(i==1|i==4|i==7|i==10){axis(2,at=seq(0,3,.5),labels=seq(0,3,.5))}
  #if(i>1){axis(2,at=seq(0,3,.5),labels=F)}
  kd_y<-kd_syn_err$y
  kd_y[kd_y<=pad]<-pad
  lines(kd_syn_err$x,kd_y,col='red',lwd=3)
  if(i==1|i==4|i==7|i==10){mtext('Test',side=2,cex=2.5,line=5)}
  if(i==3){legend('topleft',c('emp','sim'),col=c('skyblue','red'),lwd=c(6,3),cex=3,bty='n')}
  if(i%in%10:12){mtext('Residual (kcfs)',side=1,line=2.5,cex=1.5)}
}

dev.off()

png(paste('h:/oroville_non-stationary/paper/figs_rev1/si/all-mth-resids_tst4c_fig8_',wy_tag,'_v-',vers,'_nreg=',noise_reg,'.png',sep=''),width=768,height=896)
par(mfrow=c(4,3),oma=c(2,5,0,0),mar=c(2,4,3,0.5),mgp=c(2.5,0.5,0),tcl=-0.2,cex.lab=2,cex.axis=2,cex.main=3,font.main=4)
...
for(i in 1:12){
  ...
  if(i==1|i==4|i==7|i==10){dens_lab='Density'}else{dens_lab=''}
  hist(err_db_4c[seas]*hym_kcfs_conv,breaks=brks,xlim=xlm,ylim=c(0,yupr),freq=F,main=mth[i],col='goldenrod1',xlab='',ylab=dens_lab,axes=F)
  axis(1,at=x_axis,labels=x_axis)
  if(i==1|i==4|i==7|i==10){axis(2,at=seq(0,3,.5),labels=seq(0,3,.5))}
  #if(i>1){axis(2,at=seq(0,3,.5),labels=F)}
  kd_y<-kd_syn_err$y
  kd_y[kd_y<=pad]<-pad
  lines(kd_syn_err$x,kd_y,col='red',lwd=3)
  if(i==1|i==4|i==7|i==10){mtext('Test+4C',side=2,cex=2.5,line=5)}
  if(i==3){legend('topleft',c('emp','sim'),col=c('goldenrod1','red'),lwd=c(6,3),cex=3,bty='n')}
  if(i%in%10:12){mtext('Residual (kcfs)',side=1,line=2.5,cex=1.5)}
}
