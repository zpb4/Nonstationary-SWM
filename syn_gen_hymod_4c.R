...
#rg_db<-readRDS('fit/rf_corr_hymod_cal_87_97_hyp-tune.rds')
...
db_err<-predict(rg_db,data=predictor_mat_4c)$predictions
err_db<-err_4c-db_err

source('GL_maineqs_mv.R')
gl_fit<-readRDS('fit/hymod-resid-fit_mv-lin-ar_val_default.rds')

sc_factor<-readRDS('fit/sc_factor_val.rds')
sc_factor<-matrix(rep(sc_factor,length(idx)),ncol=10,byrow=T)

pred_in<-predictor_mat_4c[,1:10]
...
syn_out<-foreach(m = 1:n,.combine='cbind',.packages=c('fGarch','ranger'),.inorder=F) %dopar% {
  ...
  vmat<-predictor_mat_4c

  for(i in 4:length(syn_gen)){
    dat<-predictor_mat_hist[1:2,]
    dat[1,]<-c(vmat[(i-3),1:10],syn_gen[i-1],syn_gen[i-2],syn_gen[i-3])
...
saveRDS(syn_out,'out/hymod_4c_gl-sep-mv-lin-ar_syn-error_default.rds')

sim<-matrix(rep(predictor_mat_4c[,1],n),ncol=n,byrow=F)
swm_out<-sim+syn_out
swm_out[swm_out<0]<-0

saveRDS(swm_out,'out/hymod_4c_gl-sep-mv-lin-ar_syn-flow_default.rds')
